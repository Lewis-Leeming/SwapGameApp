...
public enum gameState {PLAYERONE, FROZENBLUE, FROZENRED, PLAYERTWO}
...
public class pVp : MonoBehaviour
{
    ...
    public void onYesOrNo(int check)
    {
        ...
        if (check == 1)
        {
            ...
            if (state == gameState.PLAYERONE)
            {
                if (NoMovesSwap(blueTiles, length))
                {
                    playerOnePass = true;
                    invalidLength = -1;
                    state = gameState.PLAYERTWO;
            ...
            {
                if (NoMovesSwap(redTiles, length))
                {
                    playerTwoPass = true;
                }
                ...
                redTiles[length] += 1;
                playerHUD.SetHUD(getRedFour(), getRedThree(), getRedTwo(), getRedOne(), redImage);
                checkFinished(playerOnePass,playerTwoPass);
            }
